...
BEGIN
    ...
    -- Conditional logic to decide on rollback or commit
    IF bookingcount > 0 THEN
        -- Table is already booked, rollback the transaction
        ROLLBACK;
        SELECT CONCAT("Table ",imputtablenumber," is already booked - booking cancelled") AS "Booking Status";
    ELSE
        -- Table is available, insert the new booking and commit the transaction
...
BEGIN
    ...
    -- Provide a message indicating the cancellation
    SELECT CONCAT('Booking ', inputbookingid, ' cancelled.') AS "Confirmation";
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `CancelOrder`(IN p_orderID INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`admin1`@`%` PROCEDURE `CheckBooking`(IN inputbookingdate DATE, IN inputtablenumber INT)
BEGIN
DECLARE tableStatus INT;     -- Check if the table is booked on the given date
SELECT COUNT(*)   INTO tableStatus     FROM bookings     WHERE bookingdate = inputbookingdate AND tablenumber = inputtablenumber;     -- Output the result 
IF tableStatus > 0 THEN
	SELECT CONCAT("Table ", inputtablenumber," is already booked on this date") AS Message;     
ELSE 
	SELECT CONCAT("Table ", inputtablenumber, " is available for booking on this date.") AS Message; 
    END IF; 
    END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN
    ...
    WHERE bookingID = inputbookingid;
    SELECT CONCAT("Booking ",inputbookingid," updated");
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-08-05 23:08:38
